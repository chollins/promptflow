...
def build_ui_from_config(config: Dict[str, Any]):
    ...
    async def generate_guide_action():
        ...
        try:
            ...
            # Save to file
            filename = f"stretch_guide_{first}_{last}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    ...
    with ui.card().classes("max-w-4xl mx-auto p-6 space-y-4"):
        ...
        with ui.row().classes("w-full gap-6"):
            ...
            muscle_card_container = ui.card().classes("w-1/3 p-4")
            with muscle_card_container:
                ui.label("Muscle").classes("text-lg font-semibold")
                muscle_content = ui.column()  # inner container you can clear
                muscle_content.clear()
                show_message("Info", "Select an area to load muscles.")

            # Card 3 - Stretches (initially empty; will populate)
            stretch_card_container = ui.card().classes("w-1/3 p-4")
            with stretch_card_container:
                ui.label("Stretches").classes("text-lg font-semibold")
                stretch_content = ui.column()  # inner container you can clear
                stretch_content.clear()
                # ui.label("_Select a muscle first_", parent=stretch_content)
                show_message("Info", "Select a muscle to load stretches.")
